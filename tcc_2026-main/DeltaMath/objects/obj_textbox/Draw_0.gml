...
if setup == false
{
    ...
    for (var p = 0; p < page_number; p++)
    {
		...
		for(var c = 0; c < text_length[p]; c++){
			...
			//get the current width of the line
...
if text_pause_timer <=0{
	if draw_char < text_length[page]
	{
		...
		if _check_char =="." || _check_char== "?" || _check_char == "!" || _check_char== ","{
		    ...
		    //typing sound
		    if snd_count < snd_delay_array[page]{ 
		        snd_count++;
		    }else{
		        snd_count = 0;
...
if accept_key
{
    ...
    if draw_char == text_length[page]
    {
        ...
        //destroy textbox OR go to option
        else
        {
            if option_number > 0 {
                ...
                if (global.after_dialog_action != "") {
                    switch(global.after_dialog_action) {
                        case "battle_carol":
...
if draw_char == text_lenght[page] && page == page_number - 1 && option_number > 0
{
	...
	// Toca som quando muda de opção
	if (_old_option_pos != option_pos) {
